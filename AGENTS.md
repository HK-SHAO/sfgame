...
- `app/wasm/` — WASM 引擎引导与实例化（单实例 = 单内存；产物 `app/wasm/sfengine.wasm`，gitignore）。流体+顶点批+示踪三内核同模块，主线程经 SAB 零拷贝直读流体场与示踪缓冲（`bilinearSample` 与 wasm 采样语义同构、f32 容差内一致）。**双模式按环境 SAB 可用性分流**（特性检测 = `typeof SharedArrayBuffer`，非 COI 浏览器中该全局不存在）：COI 环境（COOP/COEP 头：`public/_headers` 生产 + vite.config server/preview）走 SAB 零拷贝快路径；无 SAB 能力环境（非 COI 浏览器/WebView——WebView 架构上不支持 COI，与响应头无关）引导期清产物 shared 位回退普通内存（flags 读写单源 `wasm-shared.ts`，构建期 `patch-shared.ts` 置位/运行期清位同一套段解析；标志位纯元数据，数值语义与 golden 基线不受模式影响），worker 逐 tick 拷贝场/示踪随 frame 消息 transfer 送出，渲染层零分叉。部署目标首选能自定义响应头的托管（Cloudflare Pages）保证现代浏览器快路径；无头托管/WebView 自动落兼容路径（每帧搬运典型 ~350KB）。worker 运行期崩溃经 onFatal 事件走明示屏而非静默冻结
