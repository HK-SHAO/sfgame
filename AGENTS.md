...
- `app/wasm/` — WASM 引擎引导与实例化（单实例 = 单内存；产物 `app/wasm/sfengine.wasm`，gitignore）。流体+顶点批+示踪三内核同模块，主线程经 SAB 零拷贝直读流体场与示踪缓冲（`bilinearSample` 与 wasm 采样语义同构、f32 容差内一致）。SAB 要求跨域隔离 COOP/COEP（`public/_headers` 生产 + vite.config 的 server/preview 头已配；缺一则 worker 的 SAB postMessage 抛错、游戏卡死）——**部署目标必须是能自定义响应头的托管（Cloudflare Pages），不支持 itch.io 等纯静态托管**；main.ts 预检 `crossOriginIsolated`，缺失时明示「当前环境无法运行此游戏」（原因可能在浏览器或站点两侧）而非白屏；worker 运行期崩溃经 onFatal 事件同样走明示屏而非静默冻结
