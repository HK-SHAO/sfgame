...
- `app/wasm/` — WASM 引擎引导与实例化（单实例 = 单内存；产物 `app/wasm/sfengine.wasm`，gitignore）。流体+顶点批+示踪三内核同模块，场/示踪由 worker 逐 tick 拷贝随 frame 消息 transfer 送出（`bilinearSample` 与 wasm 采样语义同构、f32 容差内一致；拷贝成本计入 tickMs，governor 降级决策见真实耗时）。无 COOP/COEP 头与跨域隔离依赖（SAB 零拷贝快路径已于 2026-08 整体移除——收益仅 ~0.5ms/帧且 WebView/非 COI 环境零覆盖，见 pitfalls I13 终章），任意静态托管可部署（HTTP/内网 IP 等价运行）；worker 运行期崩溃经 onFatal 事件走明示屏而非静默冻结
