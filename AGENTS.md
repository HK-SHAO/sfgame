...
- `app/wasm/` — WASM 引擎引导与实例化（单实例 = 单内存；产物 `app/wasm/sfengine.wasm`，gitignore；`takeEngine` 实例池：预烘期备好、取后后台补，进关热路径零实例化）。流体内核与顶点批内核同模块，渲染零拷贝直读流体场（`bilinearSample` 与 wasm 采样逐位同构）
...
- `app/ui/` — `app.ts` 根组件（声明式装配 + syncScreen 从 URL 推导屏幕，dev 面板生命周期在此；启动预热全委托 `prewarm.ts` 自治模块）、`prewarm.ts`（高内聚：引擎实例池/音乐烘焙/渲染热身的流水线 + 运行时校验 + 状态 UI 一体，app 只挂载 `<sf-prewarm>`；运行中底部小字不遮挡，引擎+WebGL 关键校验不过弹可关警告并阻断进关，`prewarmPassed()` 门禁）、`sf-game.ts` 画布宿主（firstUpdated 建 GameController、disconnectedCallback 销毁，事件外发 hudchange/deny/sourceschange）
...
- `app/core/` — 固定步长循环、音效与反馈（离散反馈一律走 `feedback.ts` 门面 = `sfx.ts` 音频 + `haptics.ts` 震动唯一配对点；`music.ts` 背景音乐：`bakeScore` 纯函数乐谱可无头测试 + `MusicPlayer` stem 循环播放（合成内核在 `assembly/music.ts` 物理建模钢琴，试听 `bun run scripts/render-score.ts [seed]`）；stem 烘焙走 `music-bakery.ts`（IndexedDB 持久缓存二次加载零烘焙 → Worker 后台烘 → 内存单槽缓存，主线程零开销；全失败回退主线程分片烘；合成内核变更须 bump `STEM_CACHE_V`；`listStemCache`/`deleteStemCache` 供存储管理页枚举与清除）；`setFlow` 飞行风速驱动旋律层变奏；连续风声层/音乐传输由 controller 直驱 sfx）、性能治理（`governor.ts` 降级策略 / `wind.ts` 风强度与落地判定，均纯逻辑可无头测试）、通用 URL 状态模块
