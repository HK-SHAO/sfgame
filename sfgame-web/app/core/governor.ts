...
// 兜底只降 dpr 分辨率档（iOS Metal/弱 GPU 的负担主要在 GPU 侧，降粒子无益）。
// 成本口径 = 纯主线程渲染耗时：模拟在 worker 线程，其 tick 耗时与主线程帧预算无关（计入会误触降级，
// 见 pitfalls D6）。预算随速率放大（倍速慢帧是用户主动选择）；偶发卡顿不触发（150 帧容错）
