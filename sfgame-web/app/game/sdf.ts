// 地形 SDF 表达式求值器（零依赖）：sdf(x, y) = 到地表的有符号距离，>0 空气 / <0 实体。
...
const FUNCS: Record<string, (args: number[], x: number, y: number) => number> = {
  ...
  circle: (args, x, y) => {
    ...
    return Math.hypot(x - args[0], y - args[1]) - args[2]
  },
  ...
  capsule: (args, x, y) => {
    ...
    return Math.hypot(px - bx * t, py - by * t) - args[4]
  },
...
// bump/gauss 需要 x 坐标、flat/circle 等需要 (x,y)：调用约定 scratch = [实参..., x, y]，
// 即 args[3]/args[4]（5 参 capsule 为 args[5]/args[6]）承载坐标——热路径零分配的关键
class Parser {
  private src: string
  private i = 0
