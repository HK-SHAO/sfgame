// 欧拉稳定流体（Jos Stam）：浮力 → 半拉格朗日平流（单趟 + 阻尼）→ 压强投影保持无散度；热源加热上升、投影抽走体积 → 周围补充流入涌现水平风。环境风不进步流水线：预烘焙位流基场（贴地绕流，顺坡爬升），采样时按强度线性叠加；数值内核在 moon/fluid.mbt（WASM 标量，f32 存储/f64 中间量），本模块只是门面与纯计算辅助
