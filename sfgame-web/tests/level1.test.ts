...
test('初始状态：飞机在画布外高空、带初速飞入', () => {
  ...
  expect(sim.plane.y).toBeLessThan(LEVEL_1.ground(0) - 20)
...
test('无操作：飞机掠过目标上空、撞上右侧谷壁，无法通关', () => {
  ...
  for (let t = 0; t < 45; t += DT) {
    sim.step(DT)
    const p = sim.plane
    if (yAtGoalX === null && prevX < LEVEL_1.goals[0].x && p.x >= LEVEL_1.goals[0].x) {
      yAtGoalX = p.y
    }
    prevX = p.x
  }
  ...
  const circleTop = LEVEL_1.ground(LEVEL_1.goals[0].x) - 2 - LEVEL_1.goals[0].r
...
test('地形：谷地左浅右深，目标位于更深处', () => {
  expect(LEVEL_1.ground(70)).toBeGreaterThan(LEVEL_1.ground(5) + 4)
  // 目标圈中心比入场高度低得多（这是一次"降落"）
  const goalCenterY = LEVEL_1.ground(LEVEL_1.goals[0].x) - 2
  expect(goalCenterY).toBeGreaterThan(LEVEL_1.spawn.y! + 20)
})
