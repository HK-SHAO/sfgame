...
test('无操作：飞机缓缓滑翔坠落谷底，无法抵达目标', () => {
  ...
  for (let t = 0; t < 20; t += DT) {
    sim.step(DT)
    if (landedAt < 0 && sim.plane.y > LEVEL_2.ground(sim.plane.x) - 1.2) landedAt = t
  }
...
test('离地进入目标圈即过关', () => {
  const sim = new LevelSimulation(LEVEL_2)
  sim.plane.x = LEVEL_2.goals[0].x
  sim.plane.y = LEVEL_2.ground(LEVEL_2.goals[0].x) - 3
  sim.plane.vx = 0
  sim.plane.vy = 0
  sim.step(DT)
  expect(sim.phase).toBe('won')
...
test('基准策略可通关：沿谷底→崖脚→崖顶→目标放置热源，飞机起飞并飞行抵达', () => {
  ...
  expect(sim.plane.y).toBeLessThan(LEVEL_2.ground(sim.plane.x) - 1)
}, 30000)
