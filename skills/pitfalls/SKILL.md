...
### I9 整数字面量相除会截断（2026-08，#23 逐位对照抓出；C 移植同样踩坑）
**症状**：内核移植（AssemblyScript/C）的几何代码与 JS 逐位对照时，插值系数（如 `7/27`）产出 0 而非 0.259——渐变环颜色全错，画面却"看起来正常"，目测无法发现。
**根因**：静态类型语言：`7 / 27` 两操作数推断为整型，整数除法截断为 0；JS 同写法是浮点除。AS 推断 i32、C 默认 int，行为相同。
**修法**：显式浮点 `<f64>7 / 27`（AS）或 `7.0 / 27`（C）。移植 JS 数值代码后必须跑逐位对照（旧/新实现同场景比对 Float32Array），别靠目测。C 侧参考 `native/batch.c` 的 `t1 = 7.0 / 27`。
**信号**：内核移植后视觉"差不多但不对"、逐位对照出现整常数差异。
