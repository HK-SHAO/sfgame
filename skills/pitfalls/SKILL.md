...
- 跨线程传了用不上的大场（堆锯齿、真机无感）：先算传输/消费量级比，重构前先真机验证 → G22
...
### G22 跨线程传参纪律：传输量要与消费量成量级对应（2026-08 实测，方案已回滚）
**症状**：帧率不降，但主线程 `usedJSHeapSize` 秒级锯齿（6↔39MB）；肉眼无卡顿，真机无感知差异。
**根因**：worker 把 5 个全场 + 全部示踪缓冲逐 tick 全量 transfer（919KB × 60Hz ≈ 54MB/s），而主线程对场的实际消费仅 ~400 次双线性采样（12µs）——传输量大消费量两个量级；transfer 虽免复制，但每帧被 detach 的 ~1MB 在主线程成 GC 垃圾。
**方案与回滚**：曾实施 worker 预计算（着色批直写内核记录布局 + 旗面风留 worker 侧，919KB→~130KB，headless 实测主线程 GC 锯齿减半、tick/batch 持平）；**真机无明显提升，已回滚**（d0072db）——现代移动端 GC 对 detach 缓冲的回收开销远小于理论值，为此付出的"worker 耦合批次记录布局 + 双份着色逻辑"代价不划算。
**信号**：任何跨线程逐帧传输先算「每秒字节数 ÷ 接收方消费量」；但**先真机验证再决定是否重构**——headless 的 GC 指标不等于真机体验。
