...
- 开局数秒后必现一次百毫秒级卡顿（每局一次、长任务/LoAF 定位单帧 rAF 77ms+） → D6
...
- 多实例批只渲染一个/实例间闪现横跳（淡出后异地满显，云瞬移） → D19
- 云/装饰物被吹出可视区后长时间不出现 → G13
- 装饰物在地图边缘堆积/长期停滞，或贴着另一朵生成 → G15、G16
- 高倍速切回低倍速延迟数秒生效（仍快转/仍低帧率） → G17
- 高倍速运行中点暂停/增删源延迟数秒、恢复无延迟、画面不卡 → G21
- 帧率正常但主线程堆秒级锯齿/移动端偶发微卡（跨线程传了用不上的大场） → G22
...
### D6 自适应降级要防误触发
帧开销 EMA（平滑 0.95）+ 慢帧计数（如持续 150 帧超 13ms 才降级）——偶发卡顿不降级；只降 dpr 档（粒子数恒定全平台视觉一致，粒子是核心视觉不可降）。
**成本口径 = 纯主线程渲染耗时**：模拟迁 worker 后其 tick 耗时与主线程帧预算无关，计入会误触降级——实测 worker tick 28ms+渲染 6ms 恒超 13ms 预算，150 帧（≈5s）后触发一次 dpr 降级 → `fit(force)` 强制 resize+地形顶点批全量重烘焙 ≈ 80ms 卡顿（LoAF 定位：单个 rAF 回调 77ms），**每局必现一次**。修法：governor 只喂主线程渲染耗时；fit 加 w/h/dpr 幂等守卫（无实际变化跳过 resize），dpr=1 设备降级也不白卡。
**倍速下帧预算按速率放大**（预算 × rate）：倍速慢帧是用户主动选择，慢帧是预期而非故障。**追赶封顶**：单帧最多消化 24 模拟步（≈0.4s 模拟，loop.ts MAX_TICKS_PER_FRAME），暂停回归在 16× 下不会单帧冻结。
...
### D19 顶点批循环守卫单位混用：只画第一朵可见云（2026-08-14 实踩）
**症状**：动态实例批（云）出现「淡出后另一朵满不透明度异地闪现」的瞬移观感，同批可见实例数恒为 1；还叠加"濒死云与新生云在阈值两侧横跳"的频闪。
**根因**：`for (i < count && n < CAP)` 的 `n` 是**浮点写入计数**（每实例 36 个），`CAP` 却是实例数——写完第一个可见实例 `n=36` 即越限提前结束；隐形实例 `continue` 后 `n` 不变，又让下一个可见实例顶上 → 实例间硬切换。守卫是"容量钳"防御性添加时单位写错的（CR 引入回归）。
**修法**：钳制必须同单位（`n < CAP × 每实例浮点数`）；填充逻辑抽纯函数（`app/render/cloud-batch.ts`）由白盒测试钉死"全可见 = 全写入、隐形不占容量"。
**信号**：批内多实例却只见一个、且消失/出现在不同实例间横跳——先查循环守卫单位；任何"n 数的是 A 却和 B 比"的循环都照此查。
...
### G21 主→worker 无背压 → 高倍速交互秒级延迟（2026-08 实测）
**症状**：高倍速（4×/8×/16×）"运行中"点击暂停或增删冷热源，动作延迟数秒才生效；从暂停**恢复**则无延迟；页面本身不卡（60fps）。
**根因**：GameLoop 每帧把 rate×tick 条 tick 消息发往 worker；worker 逐条同步处理（每条 ≈ 流体内核耗时）。设备内核吞吐低于倍速要求时，tick 在 worker 消息队列**无界积压**，交互消息（place/remove/pause）FIFO 排在积压之后——延迟 = 积压 × 单 tick 耗时，随"运行中"时长增长。暂停时 tick 走冻结空转路径（亚毫秒），队列恒空 → 恢复无延迟。页面流畅是因为 MAX_ACC 丢时间（时间膨胀）+ 渲染在主线程。
**修法（已落地）**：controller 用帧快照回执（每 tick 1:1）计数在途 tick，达 `MAX_TICKS_PER_FRAME`（24，loop.ts 导出）即停发——积压上界 24 条，交互延迟上界 = 24 × 单 tick 耗时（Android ≈100-150ms）；停发时 acc 照常扣减 = 时间膨胀，与既有 MAX_ACC 语义同源，能跟上的设备零影响。
**信号**：「倍速下交互卡、画面不卡」「暂停有延迟、恢复无延迟」——先查消息积压（模拟速率 vs 墙钟），别查渲染。

### G22 跨线程传参纪律：传输量与消费量要成量级对应（2026-08 实测）
**症状**：页面帧率不降，但主线程 `usedJSHeapSize` 呈秒级锯齿（6↔39MB），移动端偶发微卡；肉眼查不出问题点。
**根因**：SAB 移除后，worker 把 5 个全场 + 全部示踪缓冲**逐 tick 全量 transfer**（919KB × 60Hz ≈ 54MB/s），而主线程对场的实际消费只有 ~400 次双线性采样（12µs）——传输量大消费量两个量级；transfer 虽免复制，但**每帧被 detach 的 919KB 在主线程成 GC 垃圾**，minor GC 每 ~1s 回收 20-30MB。
**修法（已落地）**：渲染所需的一切由 worker 预计算——示踪着色批直写内核记录布局（同 stride/cap，canary 钉死）随帧送出，旗面风采样留 worker 侧，帧载荷 919KB → ~130KB（拖尾几何不可省），主线程 GC 锯齿减半；worker 侧采样用与主线程同源的 `bilinearSample`（同输入同输出，逐位等价）。
**信号**：任何跨线程逐帧传输，先算"每秒字节数 ÷ 接收方实际消费量"——大于 ~10× 即过度传输，把"算好了再传"而不是"传过来再算"；排查主线程 GC 用 `performance.memory.usedJSHeapSize` 秒级采样看锯齿。
